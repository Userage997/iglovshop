...
const SHOP_CONFIG = {
    ...
    version: "1.1.0" // Оптимизированная версия
};
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Периодическое обновление (редко)
    setInterval(() => {
        if (document.querySelector('#screen-2.active')) {
            console.log('[SYSTEM] Auto-refreshing products...');
            loadProducts(true); // silent mode
        }
    }, SHOP_CONFIG.cacheTime * 2); // Раз в 10 минут
});

// Печатающий текст в футере (оставляем, это легкий эффект)
